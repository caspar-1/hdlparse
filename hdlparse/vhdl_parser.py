...
vhdl_tokens = {
    ...
    'port_param_type': [
        (r'\s*(in|out|inout|buffer)\s+(\w+)\s*\(',
         'port_array_param_type', 'array_range'),
...
class VhdlComponent(VhdlObject):
    ...
    def dump(self):
        ...
        for p in self.ports:
            print('\t{} ({}), {} ({})'.format(p.name, type(
                p.name), p.data_type, type(p.data_type)))
...
def parse_vhdl(text):
    ...
    for pos, action, groups in lex.run(text):
        if action == 'metacomment':
            realigned = re.sub(r'^#+', lambda m: ' ' *
                               len(m.group(0)), groups[0])
        ...
        if action == 'section_meta':
            ...
            if kind == 'function':
                vobj = VhdlFunction(name, cur_package,
                                    parameters, groups[0], metacomments)
            else:
                vobj = VhdlProcedure(
                    name, cur_package, parameters, metacomments)
            ...
            vobj = VhdlComponent(name, cur_package, ports,
                                 generics, dict(sections), metacomments)
            objects.append(vobj)
            last_item = None
            metacomments = []

        elif action == 'end_entity':
            vobj = VhdlEntity(name, cur_package, ports,
                              generics, dict(sections), metacomments)
            objects.append(vobj)
            last_item = None
            metacomments = []
            ...
            vobj = VhdlConstant(
                groups[0], cur_package, groups[1], metacomments)
            objects.append(vobj)
            kind = None
            name = None
            metacomments = []
...
def subprogram_prototype(vo):
    ...
    if isinstance(vo, VhdlFunction):
        if len(vo.parameters) > 0:
            proto = 'function {}({}) return {};'.format(
                vo.name, plist, vo.return_type)
        else:
...
class VhdlExtractor(object):
    ...
    def _register_array_types(self, objects):
        ...
        types = [o for o in objects if isinstance(
            o, VhdlType) and o.type_of == 'array_type']
        for t in types:
